...
if (isset($_POST['submitEvento'])) {
	...
	$result = mysql_query("INSERT INTO eventos(nombre, descripcion, logo) VALUES ('" . $nombre . "','" . $descripcion . "','" . $logo . "','" . $_SESSION['userID'] . "','$estado');", $link);

	if($result){
		echo "OK, evento registrado exitosamente";
	}else{
		echo "Error";
	}
		...
		<div class="formulario">
			<form action="crearcoleccion.php" method="POST" id="register-form" novalidate="novalidate" enctype="multipart/form-data">
				...
				<div class="contenedorBotonEnviar">
					<div>
						<input type="submit" name="submitEvento" class="boton" value="Submit" style="width:200px"></input>
					</div>
				</div>
			</form>
		</div>
